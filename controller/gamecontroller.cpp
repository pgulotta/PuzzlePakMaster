...
bool GameController::isProblemSolved(QVariantList problemPositionPoints,  QVariantList solutionPositionPoints,
  ...
  if ( mPuzzlePiecesSolutionAnalyzer.isPuzzleSolved(problemPositionPoints, solutionPositionPoints, mShapesFactory.acceptableVariance())) {
    return  mPuzzlePiecesSolutionAnalyzer.isPuzzleSolved(problemWorldPoints, solutionWorldPoints, mShapesFactory.acceptableVariance());
  } else {
    return false;
  }
}
