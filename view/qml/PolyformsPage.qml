          ...
          onReleased: {
            if (pressedBodyId != null) {
              ...
              var isSolved = GameController.isProblemSolved(
                    problemPositionPoints, solutionPositionPoints,
                    problemCenterPoints, solutonCenterPoints)
              if (isSolved) {
                stopTimerText()
                playMusicId.tryPlaySoundEffect()
                solutionParticleEmitterId.emitParticles(mouseX, mouseY)
        ...
        ImageScreenBoundary {
          ...
          Repeater {
            ...
            delegate: Polyform {
              ...
              z: 10
              xPosition: (drawUnit * Math.random() * 4.0) + drawUnit
              yPosition: (drawUnit * Math.random() * 4.0) + drawUnit
              world: solutionWorldId
              piecesIndex: index
              fixedRotation: true
              onPolyformSelected: {
  ...
  function populatePoints() {
    if (problemPositionPoints === null) {
      ...
      for (i = 0; i < solutionModelId.count; i++) {
        ...
        problemCenterPoints[i] = problemModelId.itemAt(i).body.getWorldCenter()
      }
    }
    for (i = 0; i < solutionModelId.count; i++) {
      solutionPositionPoints[i] = Qt.point(solutionModelId.itemAt(i).x,
                                           solutionModelId.itemAt(i).y)
      solutonCenterPoints[i] = solutionModelId.itemAt(i).body.getWorldCenter()
