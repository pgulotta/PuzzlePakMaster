...
ScrollView {
  ...
  onRowsVisibleChanged: {
    ...
    if (rowsVisible > 1 && visible)
      flickableItemId.contentY = drawUnit + flickableItemId.contentHeight - flickableItemId.height
  }
    ...
    Column {
      ...
      Repeater {
        ...
        delegate: Row {
          ...
          enabled: !(index < rowsVisible - 1 || state === Constants.stateSolved)
          Rectangle {
            id: colorMatchCountRectId
            width: drawUnit
            height: drawUnit
            color: "transparent"
          ...
          RowLayout {
              ...
              delegate: RoundedButton {
                ...
                MouseArea {
                  ...
                  onClicked: {
                    ...
                    colorsAndPositionsMatchedId.text = calculateColorPositionsMatched(
                          proposedColorsList, solutionColorsList)
            ...
            ButtonControl.Button {
              id: validateSolutionId
              visible: index === rowsVisible - 1
              Layout.preferredWidth: drawUnit + mediumPadding
              Layout.preferredHeight: drawUnit + mediumPadding
              ...
              style: ButtonStyle {
                ...
                background: Rectangle {
                  radius: validateSolutionId.width / 2
                  border.width: rectRadius
                  border.color: Material.accent
                  color: validateSolutionId.hovered ? (validateSolutionId.pressed ? Material.foreground : Material.background) : Material.primary
                }
              }
              onClicked: {
                if (isSelectionValid()) {
                  currentScoreChanged(puzzleColorsGridId.rowsVisible)
                  if (isPuzzleSolved()) {
                    puzzleSolved()
                  } else {
                    rowsVisible++
